#' @rdname calcGDPPast
calcPopulationFuture <- function(futureData = "SSPs-UN_PopDiv") { # nolint
  # Check user input
...
calcInternalPopulationFuture <- function(futureData) { # nolint
  data <- switch(
    futureData,
...
toolPopulationFutureSDPs <- function(sdps = c("SDP", "SDP_EI", "SDP_MC", "SDP_RC")) {
  popSSP1 <- readSource("SSP", "pop", "SSP1") # nolint
...
toolLabourFutureSDPs <- function(sdps = c("SDP", "SDP_EI", "SDP_MC", "SDP_RC")) {
  labSSP1 <- readSource("SSP", "lab", "SSP1") # nolint
