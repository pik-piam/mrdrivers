...
calcGDPPast <- function(pastData = "WDI-MI-James", extension1960 = "MI-James") {
  ...
  # If required, project GDP series back to 1960 using growth rates from the MI and James data sets.
  if (extension1960 == "MI-James") {
    miGDP <- calcOutput("InternalGDPPast", pastData = "MI", aggregate = FALSE, supplementary = TRUE)
    jamesGDP <- calcOutput("InternalGDPPast", pastData = "James", aggregate = FALSE, supplementary = TRUE)
    data <- toolHarmonizeFuture(past = toolListFillWith(list(miGDP, jamesGDP)), future = data, method = "growth")
